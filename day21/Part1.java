...
public class Part1
{
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        int result = codes.stream().map(code -> calcButtons(code)).reduce(0, (sum, x) -> sum + x);
    ...
    private static int dfs(List<Character> sequence, int level, boolean isNumpad)
    {
        ...
        for (int i = 0; i < sequence.size() - 1; i++) {
            ...
            result += (level == 0)
                ...
                : paths.stream().map(p -> dfs(p, level - 1, false)).min(Integer::compare).orElseThrow();
        }
    ...
    private static int calcButtons(String code)
    {
        var btns = code.chars().mapToObj(c -> (char) c).collect(Collectors.toList());
        int buttons = dfs(btns, 2, true);
