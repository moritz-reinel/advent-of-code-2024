...
public class Part2
{
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        // same procedure, starting from the finish point
        todo = new PriorityQueue<PointCost>();
        for (int i = 0; i < DIRS.length; i++) todo.add(new PointCost(new Point(tx, ty, i), 0));

        visited = new HashSet<>();
        Map<Point, Integer> costsReverse = new HashMap<>();

        while (!todo.isEmpty()) {
            var next = todo.poll();

            costsReverse.putIfAbsent(next.pos, next.cost);

            if (!visited.add(next.pos)) continue;

            int nx = next.pos.x + DIRS[(next.pos.dir + 2) % 4].x;
            int ny = next.pos.y + DIRS[(next.pos.dir + 2) % 4].y;
        ...
        Set<Dir> path = new HashSet<>();

        for (int y = 0; y < map.length; y++) {
            for (int x = 0; x < map[0].length; x++) {
                for (int dir = 0; dir < DIRS.length; dir++) {
                    var here = new Point(x, y, dir);
                    if (costs.containsKey(here) && costsReverse.containsKey(here) && (costs.get(here) + costsReverse.get(here)) == result) {
                        path.add(new Dir(x, y));
                    }
                }
            }
        }
